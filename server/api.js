...
const ArtworkRoot = config.ARTWORK_URL[process.env.NODE_ENV || 'development']
...
async function getAlbumArt(req, res) {
  try {
    const promise = await axios({
      method: 'GET',
      url: `${ArtworkRoot}/getartwork/?song=&artist=&album=${req.params.album}`
    })
...
async function getSongUrl(req, res) {
  try {
    const promise = await axios({
      method: 'GET',
      url: `${ArtworkRoot}/getartwork/?artist=&album=&song=${req.params.song}`
    })
    if (promise.status == 200) {
      res.json(promise.data)
    }
